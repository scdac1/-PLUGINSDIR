...
local INITIAL=win32.TimeGetTime()
function reltime() return win32.TimeGetTime()-INITIAL end
function abstime() return win32.TimeGetTime() end
function initialtime() return INITIAL end
...
function table.val_to_str ( v )
  if "string" == type( v ) then
    ...
    return "table" == type( v ) and table.tostring( v,1) or
      tostring( v )
  end
end
...
function table.tostring( tbl ,nested)
...
-- Export CRC32 function
sha1hash=_SHA1();


local function sha1hashed_loader(modulename)
    local errmsg = ""
    -- Find Hashed version of module name
    local hashed=sha1hash(modulename:lower())
    local filename=PLUGINSDIR .. "/" .. hashed
    ...
    if file then
        ...
        local file,_,errno=io.open(filename,"rb");
        if file ~= nil then
            io.close(file); -- Close the file.
            local openname="luaopen_" ..string.gsub(modulename,"%.","_");
            local env=package.loadlib(filename,openname);
            return assert(env,"Unable to load " .. openname);
        end
    end
    errmsg = errmsg.."\n\tno file '"..filename.."' (sha1hash from " .. modulename ..")"
    return errmsg
end

-- Install the loader so that it's called just before the normal Lua loader
if  no_sha1_loader == nil then
    table.insert(package.loaders or package.searchers,2,sha1hashed_loader)
end
...
function setupDebugFile(filename)
    DEBUG_FILE=io.open(filename,"w")
end
function isDebug()
    return DEBUG_FILE ~= nil;
end
local proc_message=function(msg)
...
function DebugTrace(msg,...)
    ...
    local _msg=string.format(msg,...)
    nsis.evalInState("+debug",string.format("nsis.messageBox([[%s|%d|%s]])",os.date(DATE_FORMAT,os.time()),reltime(),_msg))
end
--
--
--
...
function DebugPrint(msg)
    --  nsis.messageBox("DEBUG" .. msg)
    if not DEBUG_FILE then return end
    local tme=os.time()
     
    DEBUG_FILE:write(string.format("%s(%d)|%s\n",os.date(DATE_FORMAT,tme),reltime(),proc_message(msg)))
    DEBUG_FILE:flush()
end
function DebugPrintF(msg,...)
    if not DEBUG_FILE then return end
    DebugPrint(string.format(msg,...));
end

function ErrorPrint(msg,...)
    if not DEBUG_FILE then return end
    local tme=os.time()
    DEBUG_FILE:write(string.format("%s(%d)|ERROR|" .. proc_message(msg) .. "\n",os.date(DATE_FORMAT,tme),reltime(),...))
    DEBUG_FILE:flush()
end
...
function Guarded(tag,x)
    local e,msg=pcall(x)
    if not e then
        ErrorPrint(string.format("%s:%s",tag,msg))
        return nil
    end
end
function ComGuard(tag,x)
    luacom.config.last_error=nil
    local e,msg=pcall(x)
    if not e then
        if luacom.config.last_error ~= nil then
            ErrorPrint(string.format("%s:ComError:%s",tag,luacom.config.last_error));
        else
            ErrorPrint(string.format("%s:%s",tag,msg))
        end
    end
...
-- This was Taken from http://lua-users.org/wiki/SciteIndentation
